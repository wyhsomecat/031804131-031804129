...
print('本次游戏使用的图像是',file_names[x])

# 从这里开始调整图片大小格式
w = 300
h = 300
# path = file_names[x]
oldpic = rootdir + "/" + file_names[x]
jpgpath = dirs + "/temp/" + file_names[x]
copyfile(oldpic,jpgpath)

#由于函数限制，需要将JPG文件转为合适大小的PNG文件
changeJpgToPng(w, h, jpgpath)
pngpath = dirs + "/temp/" + file_names[x][0:-len('.jpg')] + 'min.png'
...
Pics = []
for i in range(9):
    filename = dirs + "/temp/" + file_names[x][0:-len('.jpg')] + 'min' + str(i) + ".png"
...
cv = Canvas(maingame, bg='blue', width=WIDTH, height=HEIGHT)
bl = Button(maingame, text="重新开始", command=callBack2, width=20)
#b2 = Button(root, text = "返回菜单",command = callBack1,width = 20)
b3 = Label(maingame, image=photo1, width=200)
label1 = Label(maingame, text="步数：" + str(steps), fg="red", width=40)
label1.pack()
...
bl.pack()
#b2.pack()
b3.place(y=50, width=150, height=150)
play_game()
drawBoard(cv)
